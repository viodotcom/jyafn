...
def make(obj: Any) -> Ref:
    """
    Inserts a Python object as a constant in the current graph, if the object is not an
    `fn.Ref`. Else, returns the passed object.
    """
...
class ResourceType:
    """
    A `ResourceType` creates resources of a given type. Think of this as the "class object"
    of resources.
    """

    @staticmethod
    def from_json(json: str) -> ResourceType:
        """
    ...
    def load(self, name: str, data: bytes) -> LazyResource:
        ...
        operation is lazy. The returned object is a `LazyResource`, that will only create
        the actual resource when a resource method from it is actually called.
        """

class LazyResource:
    """
    A `LazyResource`. This is just a lazy (a.k.a. deffered) way to define a resource
    without needing to immediately bind it to a graph. As soon as this resource is used in
    a graph, it will build the actual mapping in that graph.

    Each resource type defines its own methods, which can be accessed via the `__getattr__`
    dunder method (Python does this automatically for you).
    """

    def __getattr__(self, name: str) -> LazyResourceCall:
        """
        Creates a lazy (a.k.a deferred) method call to a method of this object. If the
        method cannot be resolved, it throws a `ValueError`.
        """

class LazyResourceCall:
    """
    A lazy (a.k.a deferred) method call to a resource.
    """

    def __call__(self, **kwds: Any) -> Any:
        """
        Actually builds the JAYFN resource method call and binds it to the current graph.
        """
...
class Extension:
    ...
    You can manage and install extensions using the `jyafn` cli. For example:
    ```bash
    jyafn ext get http://example.org/extension.so
    ```
    """
    ...
    def get(self, resource_name: str) -> ResourceType:
        ...
        requested resource is not provided by the extension.
        """
